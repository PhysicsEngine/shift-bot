...
const LINE_BOT_API_URL = 'https://trialbot-api.line.me';
/**
...
class LineBot extends EventEmitter {
   ...
   * @param  { Function } params.apiRouter   Router for API
   */
  constructor ({ channelID, channelSecret, MID, apiRouter }) {
    super();
    this._init({ channelID, channelSecret, MID, apiRouter });
  }

  /** @private */
  _init ({ channelID, channelSecret, MID, apiRouter }) {
    /**
     * Configuration.
     * @type     { Object }
    ...
    // TODO: Remove because not necessary any more
    this._fetcher = null;
    ...
    // Set API other than LINE bot receiving and sending.
    this._express.use('/api/v1', apiRouter());

    this._express.use((req, res, next) => {
      winston.debug('Validation of LINE Signature');
      const isValid = this._checkSignature({
        signature: req.header('X-LINE-ChannelSignature'),
        body: req.body
      });
      winston.debug('Validation result: ' + isValid);
    ...
    this._express.all('/callback', (req, res, next) => {
      winston.debug("Callback API for shift-bot");
      if (!req.json || !req.json.result) {
        return next(new Error('Invalid JSON'));
      }
      res.status(200);
      this.emit('receive', req.json.result);
      winston.log('debug', req.json.result);
  ...
  _checkSignature ({ signature, body }) {
    winston.log('debug', "Signatures for validation: %s, %s", signature, body);
    const hmac = createHmac('sha256', this.botConfig.channelSecret);
    winston.log('debug', "HMAC: %s", hmac);
    hmac.update(body);
  ...
  postText ({ user, users, message }) {
    winston.debug('postText: ', user, users, message);
  ...
  _postMessage ({
    ...
    winston.debug('_postMessage: ' ,userList, text, contentMetadata);
    this._options['body'] = {
      to: userList,
    ...
    request.post(this._options, function(error, response, body) {
      if (!error && response.statusCode == 200) {
        winston.log('debug', body);
      } else {
        winston.log('debug', 'error: ' + JSON.stringify(response));
      }
    });
  }
